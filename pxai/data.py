...
# Reproduces the original hard-coded behaviour exactly, so configs without an
# `augment` block are unaffected.
#
# NOTE the original ColorJitter(0.2, 0.2, 0.2) leaves HUE at its default of 0. Hue is
# precisely the axis that differs between microscopes and stains, and colour cast is
# the residual shortcut left after ROI cropping.
_DEFAULT_AUG = {
    "hflip": True,
    "vflip": False,
    "rotation": 15,
...
def build_transforms(img_size: int, train: bool, augment: dict | None = None, letterbox: bool = False):
    ...
    must be removed at test time too.
    """
...
def build_loaders(cfg) -> Loaders:
    ...
    # persistent_workers avoids 120 rounds of worker spawn; prefetch_factor stages
    # batches so the GPU is not waiting on JPEG decode. Neither changes what the
    # model sees.
