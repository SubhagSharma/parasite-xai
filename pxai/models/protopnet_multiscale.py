...
class MultiScaleProtoHead(nn.Module):
    ...
    def _similarities(self, feats: List[torch.Tensor]):
        ...
        """
        B = feats[0].shape[0]
        # float32 explicitly: under autocast feats are fp16 but max_pool2d returns
        # fp32, and an fp16 destination rejects an fp32 index-put.
        pooled = torch.zeros(B, self.P, device=feats[0].device, dtype=torch.float32)
        maps = []
        for si, s in enumerate(self.stages):
            ...
            pooled[:, idx] = F.max_pool2d(sim, sim.shape[-2:]).flatten(1).float()
            maps.append(sim)
        return pooled, maps
...
def push_multiscale(model, loader, device, max_batches=None):
    ...
    # stage_of is a registered buffer (CUDA); missed derives from best_d (CPU).
    stage_cpu = head.stage_of.detach().cpu()
    per_stage = {int(s): int(((stage_cpu == si) & ~missed).sum())
                 for si, s in enumerate(head.stages)}
