...
def families_from_csv(path: str, names=None):
    """Group concepts by morphological family.

    The CSV stores categoricals as ONE column with string values; load_concept_table
    one-hot expands them (9 raw columns -> 23 concepts). Families are reconstructed
    from that expansion so `shell_texture=smooth` and `=striated` share a family.
    """
    import csv
    with open(path) as f:
    ...
    for j, col in enumerate(header):
        ...
        if set(vals) <= {"0", "1"}:
            ...
            expanded += [col] * len(vals)
    fam, seen = [], {}
    for base in expanded:
        if base not in seen:
            seen[base] = len(seen)
        fam.append(seen[base])
    return fam
