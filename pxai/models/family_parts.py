...
class FamilyPartHead(nn.Module):
    ...
    def forward(self, feat, concept_intervene=None):
        ...
        B = feat.shape[0]
        # float32 explicitly: under autocast `feat` is fp16 but the linear below
        # returns fp32, and an fp16 destination rejects an fp32 index-put
        c_logit = torch.zeros(B, self.K, device=feat.device, dtype=torch.float32)
        c = torch.zeros(B, self.K, device=feat.device, dtype=torch.float32)
        for i, m in enumerate(self.members):
            lg = (v[:, i] @ self.fam_w[i].t() + self.fam_b[i]).float()   # (B,|family|)
            c_logit[:, m] = lg
            # softmax when the family's values are mutually exclusive; sigmoid for
            # binary singletons
            c[:, m] = (F.softmax(lg, dim=1) if len(m) > 1 else torch.sigmoid(lg)).float()
