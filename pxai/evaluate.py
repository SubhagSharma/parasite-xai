...
def run(cfg, ckpt, resume: bool = False):
    ...
    if "accuracy" not in results:
        # Full classification report: top-1 alone hides a failed rare species in an
        # 11-class long-tail problem. macro_f1 / macro_recall weight every class
        # equally (protocol 6.4).
        clf = evaluate_classification(model, loaders.test, device, loaders.classes)
        results["accuracy"] = clf["accuracy"]
